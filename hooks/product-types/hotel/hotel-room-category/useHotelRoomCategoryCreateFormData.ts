...
export const useHotelRoomCategoryCreateFormData = (enabled = true) => {
  const query = useQuery({
    queryKey: ["hotel-room-category-create-form-data"],
    enabled,
    staleTime: 1000 * 60 * 5,
    queryFn: async () => {
      await Promise.all([]);
  ...
  return {
    ...
    // key "roomCategory" để đồng bộ với update, phòng khi thêm nguồn
    // dữ liệu về sau.
    errors: {
