import FormSection from "@/components/form/FormSection";
import { FormInput, FormSelect } from "@/components/form/form-data";
import { YachtFormSchema } from "../form/schema/core/yacht.schema";
import {
  YachtRepeatType,
  YachtTripStatus,
} from "@/types/product-types/yacht/enums";

const tripStatusOptions = Object.values(YachtTripStatus).map((value) => ({
  label: value.replace(/_/g, " ").toUpperCase(),
  value,
}));

const repeatTypeOptions = Object.values(YachtRepeatType).map((value) => ({
  label: value.replace(/_/g, " ").toUpperCase(),
  value,
}));

export default function TripsStep() {
  return (
    <>
      <FormSection
        title="Trip Information"
        description="Yacht trip schedule information"
      >
        <div className="grid gap-6 md:grid-cols-2">
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.routeId"
            label="Route ID"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.departureTime"
            label="Departure Time"
            type="datetime-local"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.arrivalTime"
            label="Arrival Time"
            type="datetime-local"
          />
          <FormSelect<YachtFormSchema>
            name="routes.0.trip.0.status"
            label="Trip Status"
            options={tripStatusOptions}
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.maxGuests"
            label="Maximum Guests"
            type="number"
          />
        </div>
      </FormSection>

      <FormSection title="Trip Schedule" description="Recurring trip schedule">
        <div className="grid gap-6 md:grid-cols-2">
          <FormSelect<YachtFormSchema>
            name="routes.0.trip.0.schedule.repeatType"
            label="Repeat Type"
            options={repeatTypeOptions}
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.schedule.daysOfWeek.0"
            label="Day Of Week"
            type="number"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.schedule.startDate"
            label="Start Date"
            type="date"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.schedule.endDate"
            label="End Date"
            type="date"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.schedule.departureTime"
            label="Departure Time"
          />
        </div>
      </FormSection>

      <FormSection title="Trip Price" description="Trip final pricing">
        <div className="grid gap-6 md:grid-cols-2">
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.price.amount"
            label="Amount"
            type="number"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.price.originalAmount"
            label="Original Amount"
            type="number"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.price.tax"
            label="Tax"
            type="number"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.price.serviceFee"
            label="Service Fee"
            type="number"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.price.discount"
            label="Discount"
            type="number"
          />
          <FormInput<YachtFormSchema>
            name="routes.0.trip.0.price.finalAmount"
            label="Final Amount"
            type="number"
          />
        </div>
      </FormSection>
    </>
  );
}
